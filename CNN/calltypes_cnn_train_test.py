...
with h5py.File('../Dataset/melFSLTrainNew.h5', 'r') as hf: # melFSLTrainNoAugCNNInput melFSLTrainNoAugCNN0606.h5 melFSLTrainNoAug
    for i in hf.keys(): 
        grp = hf[str(i)]
        data_dict = {
...
with h5py.File('../Dataset/melFSLTestNoAugImpr.h5', 'r') as hf: # calltypespec20240224.h5 melFSLTestNoAugImpr.h5 melFSLTestNew.h5
    for i in hf.keys():
        grp = hf[str(i)]
        data_dict = {
            'filename': grp['filename'][()],
...
def train(model, device, train_loader, optimizer, loss_criteria, epoch):
    ...
    results_df.to_csv(f'melFSLTrainNew.csv', index=False)
    ...
    clean_results_df.to_csv(f'melFSLTrainNew_clean.csv', index=False)
...
def test(model, device, test_loader, loss_criteria):
    ...
    # Convert the results to a DataFrame and save as a CSV file
    results_df = pd.DataFrame(results, columns=['filename', 'fold', 'target', 'files_path', 'prediction'])
    results_df.to_csv(f'melFSLTrainNew_FSLTestNoAugImpr0612.csv', index=False)
    ...
    # Now create the DataFrame with the cleaned data
    clean_results_df = pd.DataFrame(cleaned_results, columns=['filename', 'fold', 'target', 'files_path', 'prediction'])
    clean_results_df.to_csv(f'melFSLTrainNew_FSLTestNoAugImpr0612_clean.csv', index=False)
...
def training(model, train_loader, test_loader, device):
    ...
    for epoch in tqdm(range(1, epochs + 1)):
        ...
        if test_loss < best_valid_loss:
            ...
            torch.save(model.state_dict(), 'melFSLTrainNewModel0612.pt')
